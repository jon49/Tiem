...
Tiem.O = function () {
   ...
   var Jobs = function(){this.jobList = []}
   ...
   var ClockState = λ.tagged(Tiem.k.state(), [Tiem.k. in (), Tiem.k.out()])
   var clockState = _.curry(ClockState)
   var jobSkeleton = _.curry(bilby.tagged('JobSkeleton', [Tiem.k.jobId(), Tiem.k.comment(), Tiem.k.singleDay(), ClockState]))
   var job = _.curry(λ.tagged('Job', [Tiem.k.jobId(), Tiem.k.jobName(), Tiem.k.comment(), Tiem.k.singleDay(), Tiem.k.total(), ClockState]))
   
   // Validation
   var validId = function (id) {
      return Tiem.isWholeNumber(id) ? λ.success(id) : λ.failure(['ID must be a whole number'])
   }
   
   var validJobName = function (name) {
      var name_ = String(name).trim()
      return _.isEmpty(name_) ? λ.failure(['Name must not be empty']) : λ.success(name_)
   }
   
   var validJobActive = function (activeJob) {
      return _.isBoolean(activeJob) ? λ.success(activeJob) : λ.failure(['Active job must be of type Boolean'])
   }
   ...
   var validSingleDay = function (singleDay) {
      ...
      return   !_.isArray(singleDay) 
                  ? λ.failure(['Single day must be an array']) 
               : countNot24(singleDay) 
                  ? λ.failure(['Single day must have array size of twenty-four']) 
               : !areNumbers(singleDay) 
                  ? λ.failure(['Single day must only have numbers in array']) 
               : !allBetweenAbs1(singleDay) 
                  ? λ.failure(['Single day must only have numbers between -1 and 1']) 
               : λ.success(singleDay)
   }
   
   var validDate = function(date){
      var date_ = _.isString(date) ? Date.parse(date) : date
      return _.isDate(date_) ? λ.success(date_) : λ.failure(['Is not a date'])
   }
   ...
   var updateJob = function(job, comment, singleDay, inOut, date){
   ...
   var validateJob = function(id, comment, singleDay, inOut, date){
      ...
      var result bilby.Do()(
         bilby.Do()(job < id_) * name_ * comment_ * singleDay_ * total * (bilby.Do()(clockState < inOut_) * date_)
      )
   ...
   var addJob = function(job){
      ...
      var jobList = _.reject(this.jobList, function(item){
